...
plt_data <- function(dat, x, y) {
  ...
  return(
    ggplot(dat, aes(!! x, !! y, group = scenario, color = condition)) +
...
line_plots <- function(dat, idx, y_label, title, ...) {
  
  subplot_a1 <- plt_data(dat %>% filter(exp_index == idx & s1==s2), contestant, ...) +
    facet_wrap(~ type, strip.position ='right')
  
  subplot_b1 <- plt_data(dat %>% filter(exp_index == idx & e1==e2), contestant, ...) +
    facet_wrap(~ type, strip.position ='right')
  
  subplot_c1 <- plt_data(dat %>% filter(exp_index == idx & (s1 != s2 & e1 != e2)), contestant, ...) +
    facet_wrap(~ type, strip.position ='right')
  
  if (idx != 'resp2a' & idx != 'resp2b') {
    return(
...
bic_plot <- function(dat) {
  ...
  dat_predictions <- rbind(
    as.data.frame(scale(dat_predictions[dat$exp_index == unique(dat$exp_index)[1],])),
    as.data.frame(scale(dat_predictions[dat$exp_index == unique(dat$exp_index)[2],]))
  )
  dat <- dat %>% 
    select(!starts_with('prediction_')) %>% 
    cbind(dat_predictions)
  
  bic_table <- NULL
  for (exp in unique(dat$exp_index)) {
    F_model <- lmer(response ~ prediction_F + condition + (1 + condition|subject),
                    data = dat %>% filter(exp_index == exp), 
                    REML = F,
                    control = lmerControl(optimizer = 'bobyqa'))
    S_model <- lmer(response ~ prediction_S + condition + (1 + condition|subject),
                    data = dat %>% filter(exp_index == exp), 
                    REML = F,
                    control = lmerControl(optimizer = 'bobyqa'))
    E_model <- lmer(response ~ prediction_E + condition + (1 + condition|subject),
                    data = dat %>% filter(exp_index == exp), 
                    REML = F,
                    control = lmerControl(optimizer = 'bobyqa'))
    FA_model <- lmer(response ~ prediction_FA + condition + (1 + condition|subject),
                     data = dat %>% filter(exp_index == exp), 
                     REML = F,
                     control = lmerControl(optimizer = 'bobyqa'))
    NFA_model <- lmer(response ~ prediction_NFA + condition + (1 + condition|subject),
                      data = dat %>% filter(exp_index == exp), 
                      REML = F,
                      control = lmerControl(optimizer = 'bobyqa'))
    BA_model <- lmer(response ~ prediction_BA + condition + (1 + condition|subject),
                     data = dat %>% filter(exp_index == exp), 
                     REML = F,
                     control = lmerControl(optimizer = 'bobyqa'))
    EBA_model <- lmer(response ~ prediction_EBA + condition + (1 + condition|subject),
                      data = dat %>% filter(exp_index == exp), 
                      REML = F,
                      control = lmerControl(optimizer = 'bobyqa'))
  ...
  return(
    bic_table %>% 
      ...
      facet_wrap(~ exp_index, nrow = 2, scales = 'free_y',
                 labeller = as_labeller(c(`1a`='Exp 1a Bonus', `1b`='Exp 1b Bonus', 
                                          `resp2a`='Xiang et al. (2023a) Responsibility', `resp2b`='Xiang et al. (2023a) Responsibility',
                                          `2a`='Exp 2a Bonus', `2b`='Exp 2b Bonus'))) +
      theme_bw() +
      labs(x = 'Model')
  )
}
...
single_participant_bic_plot <- function(dat) {
  ...
  for (exp in unique(dat$exp_index)) {
    ...
    df <- NULL
    for (s in unique(subset$subject)) {
      ...
      if (is.infinite(bic_df$BIC)[1] != T) { 
        df <- rbind(df, data.frame(exp_index = exp, subject = s, lowest = row.names(bic_df[which(bic_df$BIC == min(bic_df$BIC)),])))
      } else {
        print(paste0('Excluded subject ', s, ' in Exp ', exp, ' due to lack of variation'))
      }
    }
    counts <- data.frame(rbind(table(df$lowest)))
    num_subject <- length(unique(df$subject))
  ...
  full_p <- full_p %>% pivot_longer(cols = !exp_index, names_to = 'model', values_to = 'p', names_pattern = '(.*)_model')
  full_ci <- full_ci %>% pivot_longer(cols = !exp_index, names_to = 'model', values_to = 'ci', names_pattern = '(.*)_model')
  full_p <- merge(full_p, full_ci) %>% 
    replace_na(list(p = 0, ci = 0))
  full_p$model <- factor(full_p$model, levels = c('F','S','E','FA','NFA','BA','EBA'), ordered = T)
  ...
  return(
    full_p %>% ggplot(aes(model, p, fill = model)) +
      ...
      facet_wrap(~ exp_index, nrow = 2,
                 labeller = as_labeller(c(`1a`='Exp 1a Bonus', `1b`='Exp 1b Bonus', `resp2a`='Xiang et al. (2023a) Responsibility', `resp2b`='Xiang et al. (2023a) Responsibility',
                                          `2a`='Exp 2a Bonus', `2b`='Exp 2b Bonus'))) +
      theme_bw() +
...
correlations <- function(dat_merge_long) {
  ...
  for (exp in unique(dat_merge_long$exp_index)) {
    for (m in unique(dat_merge_long$model)) {
      ...
      # Note: use cor.test() to get p-values
