library('tidyverse')
library('lmerTest')
library('Rmisc')
library('patchwork')
source('helper.R')

### Exp1a and Exp1b
dat <- read.csv('./../Data/exp1a+1b.csv', header = T, stringsAsFactors = T)
model_predictions <- bonus_model_predictions(dat)
dat$subject <- as.factor(dat$subject)
dat$contestant <- as.factor(dat$contestant)
model_predictions$contestant <- as.factor(model_predictions$contestant)
dat <- dat %>% mutate(type = case_when(s1 == s2 ~ 'Same strength',
                                       e1 == e2 ~ 'Same effort',
                                       T ~ 'Same force'))

# Line plots
p1_1a_bonus <- line_plots(dat, '1a', 'Bonus', 'Exp 1a Bonus', bonus)
p1_1b_bonus <- line_plots(dat, '1b', 'Bonus', 'Exp 1b Bonus', bonus)

dat <- merge(dat, model_predictions) %>% 
  dplyr::rename(response = bonus) %>% 
  arrange(exp_index, subject, scenario, contestant)

# BIC plots
p2_bonus_1a1b <- bic_plot(dat) + theme(legend.position = 'none')
p3_bonus_1a1b <- single_participant_bic_plot(dat) + theme(legend.position = 'none')

# scatter plots
dat <- read.csv('./../Data/exp1a+1b.csv', header = T, stringsAsFactors = T) %>% 
  dplyr::rename(response = bonus)
model_predictions <- bonus_model_predictions(dat)
dat_merge_long <- merge_long(dat, model_predictions)
text_labels <- correlations(dat_merge_long)

p4_1a_bonus <- cor_plt(dat_merge_long %>% filter(exp_index == '1a'), prediction, avg_response, lci_response, uci_response, text_labels %>% filter(exp_index == '1a')) +
  labs(title = 'Exp 1a Bonus', x = 'Model', y = 'Data') +
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        plot.title = element_text(hjust = 0.5),
        legend.position = 'none')

p4_1b_bonus <- cor_plt(dat_merge_long %>% filter(exp_index == '1b'), prediction, avg_response, lci_response, uci_response, text_labels %>% filter(exp_index == '1b')) +
  labs(title = 'Exp 1b Bonus', x = 'Model', y = NULL) + 
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        plot.title = element_text(hjust = 0.5),
        legend.position = 'none')

# Responsibility attribution data from Xiang et al. (2023)
dat <- read.csv('./../Data/resp_data.csv', header = T, stringsAsFactors = T) %>% 
  mutate(exp_index = case_when(exp_index == '2a' ~ 'resp2a',
                               T ~ 'resp2b'))
model_predictions <- resp_model_predictions(dat)
dat$subject <- as.factor(dat$subject)
dat$contestant <- as.factor(dat$contestant)
model_predictions$contestant <- as.factor(model_predictions$contestant)
dat <- dat %>% mutate(type = case_when(s1 == s2 ~ 'Same strength',
                                       e1 == e2 ~ 'Same effort',
                                       T ~ 'Same force'))

p1_2a_resp <- line_plots(dat, 'resp2a', 'Responsibility', 'Xiang et al. (2023a) \n Responsibility', resp)
p1_2b_resp <- line_plots(dat, 'resp2b', 'Responsibility', 'Xiang et al. (2023a) \n Responsibility', resp)  

dat <- merge(dat, model_predictions) %>% 
  dplyr::rename(response = resp) %>% 
  arrange(exp_index, subject, scenario, contestant)

# BIC plots
p2_resp <- bic_plot(dat) + labs(color = 'Model') + theme(legend.position = 'none')
p3_resp <- single_participant_bic_plot(dat) + labs(fill = 'Model') + theme(legend.position = 'none')

# scatter plots
dat <- read.csv('./../Data/resp_data.csv', header = T, stringsAsFactors = T) %>% 
  mutate(exp_index = case_when(exp_index == '2a' ~ 'resp2a',
                               T ~ 'resp2b')) %>% 
  dplyr::rename(response = resp)
model_predictions <- resp_model_predictions(dat)
dat_merge_long <- merge_long(dat, model_predictions)
text_labels <- correlations(dat_merge_long)

p4_2a_resp <- cor_plt(dat_merge_long %>% filter(exp_index == 'resp2a'), prediction, avg_response, lci_response, uci_response, text_labels %>% filter(exp_index == 'resp2a')) +
  labs(title = 'Xiang et al. (2023a) \n Responsibility', x = 'Model', y = NULL) +
  theme(strip.background = element_blank(),
        strip.text = element_blank(),
        plot.title = element_text(hjust = 0.5))

p4_2b_resp <- cor_plt(dat_merge_long %>% filter(exp_index == 'resp2b'), prediction, avg_response, lci_response, uci_response, text_labels %>% filter(exp_index == 'resp2b')) +
  labs(title = 'Xiang et al. (2023a) \n Responsibility', x = 'Model', y = NULL) + 
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = 'none')


### Exp2a and Exp2b
# BIC plots
dat <- read.csv('./../Data/exp2a+2b.csv', header = T, stringsAsFactors = T)
model_predictions <- bonus_model_predictions(dat)
dat$subject <- as.factor(dat$subject)
dat$contestant <- as.factor(dat$contestant)
model_predictions$contestant <- as.factor(model_predictions$contestant)

dat <- merge(dat, model_predictions) %>% 
  dplyr::rename(response = bonus) %>% 
  arrange(exp_index, subject, scenario, contestant)

p2_bonus_2a2b <- bic_plot(dat) + labs(color = 'Model') + theme(legend.position = 'none')
p3_bonus_2a2b <- single_participant_bic_plot(dat) + labs(color = 'Model') + theme(legend.position = 'none')

# within-subject effort difference
dat <- read.csv('./../Data/exp2a+2b.csv', header = T, stringsAsFactors = T) %>% 
  mutate(subject = as_factor(subject),
         is_secret = as_factor(is_secret))

avg_bonus_binned <- dat %>% 
  mutate(effort_binned = cut(dat$effort, c(0, 30, 60, 90, 100), c('0-30', '30-60', '60-90', '90-100'))) %>%
  group_by(exp_index, subject, condition, is_secret, effort_binned) %>%
  dplyr::summarize(average_bonus = mean(bonus)) %>% 
  pivot_wider(names_from = is_secret, values_from = average_bonus) %>% 
  mutate(bonus_diff = `0` - `1`)

p5 <- avg_bonus_binned %>% 
  ggplot(aes(effort_binned, bonus_diff)) +
  stat_summary(fun.data = 'mean_cl_boot', geom = 'errorbar', width = .1) +
  stat_summary(fun = 'mean', geom = 'point') +
  stat_summary(fun = 'mean', geom = 'line', group = 1) +
  facet_grid(condition ~ exp_index, 
             labeller = as_labeller(c('2a' = 'Exp 2a', '2b' = 'Exp 2b', 'fail' = 'Fail', 'lift' = 'Lift'))) +
  labs(x = 'Effort (%)', y = 'Within-subject bonus difference') + theme_bw()

# Scatter plots
dat <- read.csv('./../Data/exp2a+2b.csv', header = T, stringsAsFactors = T)
dat$subject <- as.factor(dat$subject)
dat$is_secret <- as.factor(dat$is_secret)

p6 <- dat %>% 
  ggplot(aes(is_secret, bonus, color = condition)) +
  stat_summary(fun.data = 'mean_cl_boot', geom = 'errorbar', width = .1) +
  stat_summary(fun = 'mean', geom = 'point') +
  scale_color_manual(name = NULL, labels = c('Fail', 'Lift'),
                     values = c('#C77CFF', '#00BAE0'),
                     limits = c('fail', 'lift')) +
  scale_x_discrete(labels = c('1' = 'Secret', '0' = 'Non-secret')) +
  # scale_y_continuous(limits = c(0,10), breaks = c(0,2.5,5,7.5,10)) +
  facet_grid(~exp_index, 
             labeller = as_labeller(c('2a' = 'Exp 2a', '2b' = 'Exp 2b'))) +
  labs(x = 'Secrecy', y = 'Bonus') + theme_bw()

# fig2
pdf('fig2.pdf', onefile = T, width = 12, height = 8)
p1_1a_bonus | p1_2a_resp | p1_1b_bonus | p1_2b_resp
dev.off()

# fig3
pdf('fig3.pdf', onefile = T, width = 12, height = 6)
p2_bonus_1a1b + p2_resp + p2_bonus_2a2b + p3_bonus_1a1b + p3_resp + p3_bonus_2a2b + plot_layout(nrow = 2)
dev.off()

# fig4
pdf('fig4.pdf', onefile = T, width = 12, height = 8)
p4_1a_bonus | p4_2a_resp | p4_1b_bonus | p4_2b_resp
dev.off()

# fig5
pdf('fig5.pdf', onefile = T, width = 8, height = 3)
p5
dev.off()

# fig6
pdf('fig6.pdf', onefile = T, width = 6, height = 3)
p6
dev.off()

